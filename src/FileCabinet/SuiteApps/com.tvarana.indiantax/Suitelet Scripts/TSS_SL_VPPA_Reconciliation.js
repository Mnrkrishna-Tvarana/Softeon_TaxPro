...
define(['N/search', 'N/ui/serverWidget', 'N/format', 'N/record'], (search, serverWidget, format, record) => {
    ...
    const onRequest = (scriptContext) => {
        try {
            ...
            //            if (scriptContext.request.parameters.downloadPdf) {
            //     response.setHeader({ name: 'Content-Type', value: 'application/json' });
            //     response.write(JSON.stringify(reportData));
            //     return;
            // }

            // Step 1: Create the form for the report
            let form = serverWidget.createForm({ title: "TDS Reconciliation Report" });
            ...
            let subsidField = form.addField({
                id: 'custpage_tss_prepayment_subs',
                label: "Vendor Pre-payment Subsidiary",
            ...
            let tdsSectionField = form.addField({
                id: 'custpage_tss_tds_section',
                label: "TDS Section Code",
                type: serverWidget.FieldType.SELECT,
                // source: 'customrecord_tss_tdsrelation',
                source: 'customrecord_tss_its_tdsmaster',
            ...
            tdsSectionField.updateBreakType({
                breakType: serverWidget.FieldBreakType.STARTCOL
            });
            ...
            form.addButton({
                ...
                functionName: 'generateVppaReconReportFile()'
            });
            // form.addButton({
            //      label: "PDF",
            //      id: 'custpage_tss_pdf_vppa_recon_report',
            //      functionName: 'generatepdfReconReportFile()'
            //  });
                ...
                <table class="tablestyle">
                    <thead>
                        <tr>
                            ...
                            <th>Vendor Pre-Payment TDS Section Code</th>
                            <th>Vendor Pre-Payment TDS Rate</th>
                            <th>Vendor Pre-Payment Gross Amount</th>
                            <th>Vendor Pre-Payment Net Amount</th>
                            <th>Vendor Pre-Payment TDS Amount</th>
...
<th>Bill TDS Amount</th>
            ...
            var vpaSearch = search.create({
                ...
                columns:
                    [
                        ...
                        search.createColumn({ name: "custbody_tss_tds_amount", label: "TDS Amount" }),
            ...
            pagedVpaResults.pageRanges.forEach((pageRange) => {
                ...
                page.data.forEach((result) => {
                    ...
                    vpaMap[id] = {
                        ...
                        reversalAmt: result.getValue({ name: 'custbody_tss_tds_amount' }) || 0,
                        vppabills: billArr
                    };
            ...
            try {
                ...
                log.audit('Request Parameters', requestParams);
                var selectedSubsid = requestParams.custpage_tss_prepayment_subs || '';
                ...
                var selectedSection = requestParams.custpage_tss_tds_section || '';
                ...
                var vppSearch = search.create({
                    ...
                    filters:
                        [
                            ...
                            ["mainline", "any", ""],
                            "AND",
                            ["custbody_tss_tds_amount", "greaterthanorequalto", "1.00"]
                        ],
                    columns:
                        [
                            ...
                            search.createColumn({ name: "custbody_tss_tds_section", label: "TDS Section" }),
                            search.createColumn({ name: "custbody_tss_tds_percentage", label: "Vendor Pre-Payment TDS Rate" }),
                            search.createColumn({ name: "custbody_tss_tds_baseamt", label: "Vendor Pre-Payment Base Amount(Gross)" }),
                            search.createColumn({ name: "custbody_tss_tds_amount", label: "Vendor Pre-Payment TDS Amount" }),
                            search.createColumn({
                            ...
                            search.createColumn({
                                name: "formulanumeric",
                                formula: "ABS({custbody_tss_tds_amount}) - ABS({applyingtransaction.custbody_tss_tds_amount})",
                                label: "Formula (Numeric)"
                            }),
                            search.createColumn({
                                name: "custbody_tss_tds_amount",
                                join: "applyingTransaction",
                                label: "Application TDS Amount"
                            }),
                            search.createColumn({ name: "custbody_tss_tds_relation", label: "TDS Type" }),
                ...
                if (selectedSubsid) subsidField.defaultValue = selectedSubsid;
                if (selectedFromDate) fromDateField.defaultValue = formattedFrom;
                if (selectedToDate) toDateField.defaultValue = formattedTo;
                if (selectedSection) {
                    // filters.push(search.createFilter({
                    //     name: 'custbody_tss_tds_relation',
                    //     operator: search.Operator.ANYOF,
                    //     values: selectedSection
                    // }));
                    filters.push(search.createFilter({
                        name: 'custrecord_tss_vedtdstype',        // field on the related record
                        join: 'custbody_tss_tds_relation',       // join from VPP to relation
                        operator: search.Operator.ANYOF,
                        values: selectedSection
                    }));


                    tdsSectionField.defaultValue = selectedSection;
                }
                ...
                pagedVppResults.pageRanges.forEach((pageRange) => {
                    ...
                    page.data.forEach((result) => {
                        ...
                        let header = {
                            ...
                            sectionCode: result.getText({ name: 'custbody_tss_tds_relation' }) || '', // custbody_prepayment_
                            tdsRelation: result.getValue({ name: 'custbody_tss_tds_relation' }),
                            // sectionCode: get('custbody_tss_tds_section'),  // custbody_prepayment_
                            rate: get('custbody_tss_tds_percentage'),      // custbody_tds_vendorprepayment_
                            // grossAmt: get('custbody_tss_tds_baseamt'),     // custbody_gross_amount
                            grossAmt: Math.abs(parseFloat(result.getValue('fxamount') || 0)),     // custbody_gross_amount

                            //grossAmt:paymentAmount,
                            tdsAmt: parseFloat(get('custbody_tss_tds_amount')) || 0,  // custbody_vendor_prepayment
                            // netAmt: result.getValue(result.columns[10]) || 0,  // Net amount from search column  intial 12 "amount field'"
                            netAmt: get('custbody_tss_tds_baseamt'),
            ...
            if (Object.keys(groupedVPP).length === 0) {
                ...
                for (let i = 0; i < orderedVppList.length; i++) {
                    ...
                    // let adjusted = Math.abs(tdsAmount - totalReversalAmt);
                    ...
                    //         appType: app?.appType || '',
                    //         // billDocNumber: matchedVPA.billDocNumber || '',
                    //         // appliedBillAmount: formatCurrency(matchedVPA.appliedBillAmount || 0),
                    //         // billTdsAmount: formatCurrency(Math.abs(parseFloat(matchedVPA.billTdsAmount || 0))),
                    ...
                    let vpaDetailsArray = apps.map(app => {
                        ...
                        return {
                            ...
                            appliedBillAmount: formatCurrency(matchedVPA.appliedBillAmount || 0),
                            // billDocNumber: matchedVPA.billDocNumber,
                            // billInternalId: matchedVPA.billInternalId,
                            // billTdsAmount: formatCurrency(Math.abs(parseFloat(matchedVPA.billTdsAmount || 0))),
                            reversalAmt: formatCurrency(parseFloat(matchedVPA.reversalAmt || 0)),
                        };
                    });
                ...
                if (finalJsonArray.length === 0) {
                    ...
                    pageSlice.forEach(entry => {
                        ...
                        apps.forEach((vpa, idx) => {
                            ...
                            // log.debug("vpa.vppabills.length",vpa.vppabills.length)
                            //Getting Bills count for vppa to add rowspan for VPP
                            ...
                            log.debug("vpa.appInternalId", vpa.appInternalId)
                            // if (vpa.appInternalId && billrowspan > 1) {
                            ...
                            <td rowspan="${billrowspan}" class="center-align">
                                ...
                                <td class="right-align">${vpa.appliedBillAmount}</td>`
                                var baseAmt = 0;
                                var tdsRelation1 = entry.tdsRelation
                                bill.billTdsObj = bill.billTdsObj ? JSON.parse(bill.billTdsObj) : {}
                                // log.debug("bill.billTdsObj", bill.billTdsObj)
                                // if (vpa.appInternalId == 3600) {
                                //     log.debug("bill.billTdsObj[tdsRelation1]", bill.billTdsObj[tdsRelation1])
                                //     log.debug("bill.billTdsObj[tdsRelation1].tdsvppa", bill.billTdsObj[tdsRelation1].tdsvppa)
                                //     log.debug("bill.billTdsObj[tdsRelation1].tdsvppa[vpa.appInternalId]", bill.billTdsObj[tdsRelation1].tdsvppa[vpa.appInternalId])
                                // }
                                ...
                                reportHtmlString += ` 
                                <td class="right-align">${billTDSamt}</td>`
                                if (bill_idx === 0) {
                                    reportHtmlString += `
                            ...
                            `;
                                ...
                                //      `
                                // }
                                //<td class="center-align">${vpa.appDocNumber}</td>
                                //<td class="right-align">${vpa.appAmount}</td>

                                // <td class="center-align">${vpa.billDocNumber}</td>
                                //     <td class="right-align">${vpa.appliedBillAmount}</td>
                                //     <td class="right-align">${vpa.billTdsAmount}</td>
